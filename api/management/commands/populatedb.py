...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if subject.count() <= 0:
            for child in root:
                for v in child:
                    c = Subject(
                        name=v.text, course=Course.objects.get(name=v.tag))
        ...
        if len(subjects):
            ...
            self.stdout.write(
                "No subjects added. Please remove already inserted subjects from db!"
            )
    ...
    def create_campuses(self):
        ...
        if c.count() <= 0:
            ...
            self.stdout.write(
                "No campuses added. Please remove already inserted campuses from db!"
            )
    ...
    def create_courses(self):
        ...
        if c.count() <= 0:
            courses_name = [
                "ENGENHARIA", "SOFTWARE", "ELETRONICA", "AEROESPACIAL",
                "ENERGIA", "AUTOMOTIVA"
            ]
            courses = []
            for name in courses_name:
                course = Course(
                    name=name, campus=Campus.objects.get(
                        pk=1))  # fix logic to add other campuses courses
            ...
            self.stdout.write(
                "No courses added. Please remove already inserted courses from db!"
            )
